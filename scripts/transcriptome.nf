...
process file_copy {
	input:
		tuple val(sampleId), file (bamFile), file(bamBai)
	script:
	"""
	...
	python3 ${params.merge_csvs_script} ${sampleId} ${PWD}/Final_Output/${sampleId}/${sampleId}.xlsx ${PWD}/arriba/${sampleId}.arriba.fusions.tsv ${PWD}/fusioncatcher/${sampleId}.fusioncatcher.fusion-genes.txt ${PWD}/fusioncatcher/${sampleId}.fusioncatcher.summary.txt ${PWD}/starfusion/${sampleId}.starfusion.fusion_predictions.tsv ${sampleId}_stringtie.tsv
	"""
}
